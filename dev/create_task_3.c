...
void handle_input(char ***symbol, char **range, char **file_name, int *symbol_count) {
    ...
    *symbol = (char**)malloc(sizeof(char*) * (*symbol_count));
    ...
    printf("Enter file name (default = data%d.csv) ", i);
    fgets(buffer, sizeof(buffer), stdin);
    buffer[strcspn(buffer, "\n")] = 0;

    if (strlen(buffer) == 0) {
        sprintf(default_file, "data%d.csv", i);
...
void write_test_file(const char* file_name, double** prices, char** symbols, int symbol_count, int count) {
...
void get_and_write_prices(char** symbol, const char* range, const char* file_name, int symbol_count) {
    ...
    write_test_file(file_name, prices, symbol, symbol_count, count);
